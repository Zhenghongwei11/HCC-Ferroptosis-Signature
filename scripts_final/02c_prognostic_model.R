...
if (length(sig_genes) >= 2) {
  ...
  pdf(file.path(plot_dir, "Supp_Figure_S8_LASSO_Selection.pdf"), width = 10, height = 4.5)
  par(mfrow = c(1, 2), mar = c(4, 4, 2.5, 1))
  plot(cv_fit$glmnet.fit, xvar = "lambda", label = FALSE, main = "LASSO-Cox coefficient paths")
  abline(v = log(cv_fit$lambda.min), lty = 2, col = "#2F4F5F")
  plot(cv_fit, main = "CV partial likelihood deviance")
  dev.off()

  png(file.path(plot_dir, "Supp_Figure_S8_LASSO_Selection.png"), width = 3000, height = 1350, res = 300)
  par(mfrow = c(1, 2), mar = c(4, 4, 2.5, 1))
  plot(cv_fit$glmnet.fit, xvar = "lambda", label = FALSE, main = "LASSO-Cox coefficient paths")
  abline(v = log(cv_fit$lambda.min), lty = 2, col = "#2F4F5F")
  plot(cv_fit, main = "CV partial likelihood deviance")
